 ...
 * @Description: 时间戳转换
 * @Author: Liu Jia
 * @Date: 2022-11-09 00:15:57
 * @LastEditTime: 2022-11-12 02:15:46
-->
<template>
  ...
  <div class="bilibili_cover_get view">
    <div main="" class="nya-container pt">
      ...
      <form class="el-form nya-input-btn">
        <el-switch
          ...
          inactive-text="豪秒"
        />
        <span class="nya-subtitle" style="margin-top: 20px" v-if="currentTime"
          >当前时间：<el-tag>{{ currentTime }} </el-tag>
        </span>
        <span
          class="nya-subtitle"
          style="margin-top: 20px"
          v-if="currentTimestamp"
          >当前时间戳：<el-tag>{{ currentTimestamp }}</el-tag>
          <vcopy :value="currentTimestamp" />
        </span>
        <el-divider />
        <span class="nya-subtitle" style="margin-top: 20px"
          >时间（北京） → 时间戳：
        </span>
        ...
        <el-row style="font-weight: 700 !important">
          ...
          <el-col :lg="6" :xs="24">
            <el-tag v-if="inputTimestamp">{{ inputTime }} </el-tag>
          </el-col>
        </el-row>
      </form>
    </div>
    ...
    <div class="nya-container pt">
      ...
      <ul class="nya-list">
        ...
        <li>
          Unix 时间戳（英文为 Unix epoch, Unix time, POSIX time 或 Unix
          timestamp）
        </li>
        ...
        <li>
          在大多数的 Unix 系统中 Unix 时间戳存储为 32 位，这样会引发 2038
          年问题或 Y2038。
        </li>
      </ul>
    </div>
